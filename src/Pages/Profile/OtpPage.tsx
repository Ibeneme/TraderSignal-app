  ...
  useEffect(() => {
    dispatch(getCurrentUser())
      .then(response => {
        ...
        console.log('User profile retrieved:', response?.payload?.user?.email);
        setEmail(response?.payload?.user?.email);
        dispatch(forgotPassword(response?.payload?.user?.email))
          .then(response => {
            console.log('Registration lolRegistration lol', response?.payload);
    ...
    dispatch(otpVerification({email: email, code: otp}))
      .then(response => {
        ...
        if (response?.payload?.msg === 'Verification Successful') {
          if (page === 'LoggedOTPScreen') {
            navigation.navigate('ChangePasswordScreen' as never);
          } else if (page === 'WithdrawalInputScreen') {
            navigation.navigate('WithdrawalInputScreen' as never);
          }
        } else {
          setLoading(false);
          // setFormErrors('Invalid OTP');
...
export default LoggedOTPScreen;

...
import {generateOTP, otpVerification, register} from '../../Redux/Auth/Auth';
...
const OTPScreen = ({}) => {
    ...
    dispatch(generateOTP(values.email))
      .then(response => {
        ...
        if (response?.payload?.otp || response?.payload?.email) {
          setResend(
            `OTP Sent Successfullly to the email address ${values.email}`,
          );
        } else {
        }
      })
  ...
  const handleVerifyOTP = ({otp}) => {
    ...
    setLoading(true);
    dispatch(otpVerification({email: values.email, code: otp}))
      .then(response => {
        console.log('Registration lol', response);
        if (response?.payload?.msg === 'Verification Successful') {
          if (values && typeof values === 'object') {
            const {confirmPassword, selectedOption, ...restValues} = values;

            const updatedValues = {
              ...restValues,
              provider:
                values.selectedOption === 'Access trading signals'
                  ? false
                  : true,
            };
            ...
            setLoading(false);
            console.error('values is not an array or is undefined');
          }
        } else {
          setLoading(false);
        ...
        <BodyView>
          <View style={{marginTop: 42}}>
            ...
            <Text
              ...
              Top Complete your account creation, Please Check your sms and
              Verify your OTP
            </Text>

...
const CheckPay: React.FC<Props> = () => {
  ...
  const {id, creator} = route.params; // Assuming userId is the name of the parameter
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      dispatch(getUsersub(id))
        .then(response => {
          setLoading(false);
          // console.log('get sub eeee:', response);
      ...
      dispatch(getWalletAddress('subscriptions'))
        .then(response => {
  ...
  const handleSubscriber = () => {
    ...
    if (selectedImage) {
      ...
      const durationInDays = subDetails?.durationInDays;
      const price = subDetails?.price;
      const subscriptionId = subDetails?.id;
      const subscriberId = thisUser?.id;
      const status = 'pending';

      const statusData = {
        durationInDays: subDetails?.durationInDays,
      ...
      console.log(thisUser, subDetails?.id, creator, 'thisUserthisUser');
      ...
      dispatch(
        ...
        .then(response => {
          ...
          if (response?.payload?.message === 'Status created successfully') {
            ...
            Alert.alert(
              'Your payment recieved succesfully awaiting confirmation',
            );
          }
        })
        .catch(error => {
  ...
  const handleSubscribe = async () => {
    ...
    if (selectedImage) {
      console.log(`let's go`, subDetails);

      const durationInDays = subDetails?.durationInDays;
      const price = subDetails?.price;
      const subscriptionId = subDetails?.id;
      const subscriberId = thisUser?._id;
      const status = 'pending';
      const newWidth = 300;
      const newHeight = 500;
      const quality = 100;
      try {
        ...
        // Create FormData object for the resized image
        const formData = new FormData();
        formData.append('image', {
        ...
        console.log(formData, 'statusData');
        setLoadingSubmit(true);
        dispatch(
          createStatus({
            durationInDays: durationInDays,
    ...
    <SafeArea>
      ...
      <BodyView paddingHorizontal={true}>
        <View style={{padding: 12, marginTop: 36}}>
          ...
          <BoldText
            textContent={`Subscribe to $${
              subDetails?.price ? subDetails.price : '$0.00'
            }`}
          />
          <Text
            style={{
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                handleCopyToClipboard(
                  `${thisUser?.email}-${subDetails?.id}-${today}`,
                )
              }>
              <Text
                style={{
            ...
            <ScrollView>
              ...
              <View style={{marginTop: 32}}>
                <TouchableOpacity
                  ...
                  onPress={handleSubscribe} // handleSubmit is already provided by Formik
